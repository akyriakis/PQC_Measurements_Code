...
void FET_Read_Info_from_File(TString Analysis_DataFileName, int id, int file_id)
{
  ...
  //}
   ...
   if((id % 2) == 0) FET_iv[id]->SetTitle(Form("VPX%d_0%s_2-S_HM_E_Left_PQC1_FET",HM_Id,Structure_Id[file_id].c_str()));
   if((id % 2) == 1) FET_iv[id]->SetTitle(Form("VPX%d_0%s_2-S_HM_W_Left_PQC1_FET",HM_Id,Structure_Id[file_id].c_str()));
...
void FET_PQC_Analysis_Final(int nFiles)
{
 ...
 //   gROOT->Reset();
   ...
   for (int i=0;i<2;i++) {
     ...
     FET_cc_final[i]->BuildLegend(0.15,0.5,0.55,0.85);
...
void Flute1_2_S_Extended_Characterization_with_xml(int nFiles)
{
   ...
   CVS_Output_File.open(Form("CSV_Info/VPX%d/Flute1_VPX%d_0xx_2-S_HM_E_W_Left_Extended_Info.csv",HM_Id,HM_Id));
